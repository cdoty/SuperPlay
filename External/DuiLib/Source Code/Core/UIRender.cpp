...
extern "C"
{
    extern unsigned char *dui_stbi_load_from_memory(unsigned char const *buffer, int len, int *x, int *y, \
...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, LPCTSTR type, DWORD mask)
{
    ...
    LPBYTE pImage = NULL;
    int x,y,n;
    pImage = dui_stbi_load_from_memory(pData, dwSize, &x, &y, &n, 4);
    delete[] pData;
	if( !pImage ) {
