...
XMLError XMLDocument::LoadFile( const char* filename )
{
	...
	if (false == fp->open(filename, true))
	{
		delete	fp;
...
XMLError XMLDocument::LoadFile( SPlay::File* fp )
{
    Clear();

    size_t size = fp->getLength();
	...
	if (false == fp->readBuffer(reinterpret_cast<uint8_t*>(_charBuffer), size))
	{
